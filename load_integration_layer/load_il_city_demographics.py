...
from constants import missing_val_replace_numeric
...
def load_city_demographics(spark, integration_layer_loc, landing_zone_name):
    try:
        ...
        delta_city_demographics.alias("oldData") \
            .merge(df_LZ_l_airline_id.alias("newData"),
                   "oldData.CITY = newData.CITY AND oldData.STATE = newData.STATE AND oldData.RACE = newData.RACE") \
        ...
        logging.info('CITY_DEMOGRAPHICS has been loaded in the Integration Layer')

    except Exception as e:
        logging.error(f'Failed to load CITY_DEMOGRAPHICS in the INTEGRATION_LAYER,{e}')
        spark.stop()
        raise Exception(f'Failed to load CITY_DEMOGRAPHICS in the INTEGRATION_LAYER,{e}')
