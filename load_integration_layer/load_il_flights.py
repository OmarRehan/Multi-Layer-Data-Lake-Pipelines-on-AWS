...
def load_flights(spark, integration_layer_loc, landing_zone_name,year,month):
    try:
        ...
        df_LZ_flights.write \
            ...
            .save(integration_layer_loc + '/FLIGHTS')

        logging.info('FLIGHTS has been loaded in the Integration Layer')

    except Exception as e:
        logging.error(f'Failed to load FLIGHTS in the INTEGRATION_LAYER,{e}')
        spark.stop()
        raise Exception(f'Failed to load FLIGHTS in the INTEGRATION_LAYER,{e}')
...
if __name__ == '__main__':
    ...
    # finding and parsing year and month args to be provided to the function
    for arg in sys.argv:
        if 'year' in arg:
            year = arg.split('=')[1]
        elif 'month' in arg:
    ...
    if (year is not None) and (month is not None):
        ...
        load_flights(spark, integration_layer_loc, landing_zone_name,year,month)

    else:
        raise Exception(
            'year and month arguments are required to start loading, ex: year=2020 month=1')
